...
object Fibers extends IOApp.Simple {
  ...
  /** Exercises:
    *   1. Write a function that runs an IO on another thread, and, depending on the result of the
    *      fiber
    *      - return the result in an IO
    *      - if errored or cancelled, return a failed IO
    *   2. Write a function that takes two IOs, runs them on different fibers and returns an IO with
    *      a tuple containing both results.
    *      - if both IOs complete successfully, tuple their results
    *      - if the first IO returns an error, raise that error (ignoring the second IO's
    *        result/error)
  ...
  def testEx1() = {
    val aComputation = IO("starting").debugD >> IO.sleep(1.second) >> IO("done!").debugD >> IO(42)
  ...
  def testEx2() = {
    val firstIO  = IO.sleep(2.seconds) >> IO(1).debugD
    val secondIO = IO.sleep(3.seconds) >> IO(2).debugD
    tupleIOs(firstIO, secondIO).debugD.void
  }
  ...
  def testEx3() = {
    val aComputation = IO("starting").debugD >> IO.sleep(1.second) >> IO("done!").debugD >> IO(42)
    timeout(aComputation, 500.millis).debugD.void
  }
