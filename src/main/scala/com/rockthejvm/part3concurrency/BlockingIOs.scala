...
object BlockingIOs extends IOApp.Simple {
  ...
  val someSleeps = for {
    _ <-
      IO.sleep(1.second).debugD // SEMANTIC BLOCKING - no threads are actually blocked, CE assigns this thread to some other fiber
    _ <- IO.sleep(1.second).debugD
  } yield ()
  ...
  val iosOnManyThreads = for {
    ...
    _ <- IO("second").debugD // the rest of this effect may run on another thread (not necessarily)
    _ <- IO.cede
    _ <- IO("third").debugD
  } yield ()
  ...
  def testThousandEffectsSwitch() = {
    ...
    (1 to 1000).map(IO.pure).reduce(_.debugD >> IO.cede >> _.debugD).evalOn(ec)
  }
